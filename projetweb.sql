...
--
-- Hôte : 127.0.0.1:3306
-- Généré le :  sam. 16 nov. 2019 à 13:29
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addpanier` (IN `membre` INT, IN `article` INT)  NO SQL
INSERT INTO panier (id_membre,id_boutique,qte) VALUES (membre,article,1)$$

DROP PROCEDURE IF EXISTS `addPhoto`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addPhoto` (IN `event` INT, IN `photo` VARCHAR(200))  NO SQL
INSERT INTO photo (id_event,url,nbr_like)
VALUES (event,photo,0)$$

DROP PROCEDURE IF EXISTS `addproduit`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addproduit` (IN `nom` VARCHAR(200), IN `description` VARCHAR(200), IN `prix` FLOAT, IN `categorie` VARCHAR(200), IN `url` VARCHAR(200))  NO SQL
INSERT INTO boutique (nom,description,prix,categorie,url)
VALUES (nom,description,prix,categorie,url)$$

DROP PROCEDURE IF EXISTS `addqte`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addqte` (IN `membre` INT, IN `article` INT)  NO SQL
UPDATE panier
SET qte = qte + 1
WHERE id_membre=membre AND id_boutique = article$$

DROP PROCEDURE IF EXISTS `addusers`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `addusers` (IN `nom` VARCHAR(200), IN `prenom` VARCHAR(200), IN `localisation` VARCHAR(200), IN `adresse_mail` VARCHAR(200), IN `mdp` VARCHAR(200))  NO SQL
...
DROP PROCEDURE IF EXISTS `allpanier`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `allpanier` (IN `membre` INT)  NO SQL
SELECT * FROM panier 
INNER JOIN boutique ON panier.id_boutique=boutique.id
WHERE id_membre=membre$$

DROP PROCEDURE IF EXISTS `allProducts`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `allProducts` (IN `categori` VARCHAR(200))  NO SQL
...
DROP PROCEDURE IF EXISTS `rempanier`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `rempanier` (IN `membre` INT, IN `article` INT)  NO SQL
DELETE FROM panier WHERE id_membre=membre AND id_boutique=article$$

DROP PROCEDURE IF EXISTS `remqte`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `remqte` (IN `membre` INT, IN `article` INT)  NO SQL
UPDATE panier
SET qte = qte - 1
WHERE id_membre=membre AND id_boutique = article$$

DROP PROCEDURE IF EXISTS `samemdp`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `samemdp` (IN `maile` VARCHAR(200), IN `motDePass` VARCHAR(200))  NO SQL
SELECT mdp
FROM membre
WHERE adresse_mail=maile AND mdp=motDePass$$

DROP PROCEDURE IF EXISTS `samepseudo`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `samepseudo` (IN `maile` VARCHAR(200))  NO SQL
SELECT *
FROM membre
...
DROP PROCEDURE IF EXISTS `selectArticle2`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectArticle2` (IN `mine` FLOAT, IN `maxe` FLOAT, IN `categori` VARCHAR(200))  NO SQL
SELECT * FROM boutique WHERE boutique.categorie=categori AND (Prix BETWEEN mine AND maxe)$$
...
DROP PROCEDURE IF EXISTS `selectpanier`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectpanier` (IN `membre` INT, IN `article` INT)  NO SQL
SELECT * FROM panier 
INNER JOIN boutique ON panier.id_boutique=boutique.id
WHERE id_membre=membre AND id_boutique=article$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `testForm` (IN `pseud` VARCHAR(200), IN `photo` VARCHAR(255), IN `titr` VARCHAR(200), IN `conten` TEXT, IN `datetim` DATETIME)  NO SQL
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `testForm2` (IN `pseud` VARCHAR(200), IN `photo` VARCHAR(255), IN `titr` VARCHAR(250), IN `conten` TEXT, IN `datetim` DATE)  NO SQL
...
INSERT INTO `panier` (`id`, `id_membre`, `id_boutique`, `qte`) VALUES
(2, 2, 7, 1),
(5, 2, 6, 2);
